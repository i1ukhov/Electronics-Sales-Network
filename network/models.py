...
class Plant(models.Model):
    ...
    address = models.CharField(max_length=200, verbose_name="Адрес", help_text="Введите улицу и номер дома")

    def __str__(self):
        return f"Завод {self.name}. {self.country}, {self.city}, {self.address}. Email: {self.email}"
...
class Product(models.Model):
    ...
    name = models.CharField(max_length=200, verbose_name='Название продукта', help_text='Введите название')
    ...
    def __str__(self):
        return f'{self.name} ({self.model}) - Завод: {self.plant.name}'
...
class Network(models.Model):
    ...
    def __str__(self):
        return f'Розничная сеть: {self.name}. Завод: {self.plant.name}. Продукт: {self.product.name}'
...
class Entrepreneur(models.Model):
    ...
    def __str__(self):
        return f'ИП: {self.name}. Завод: {self.plant.name}. Продукт: {self.product.name}'
